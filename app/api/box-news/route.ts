...
export async function GET() {
  try {
      ...
      return Response.json(
        {
          ...
          message:
            'Please configure BOX_CLIENT_ID and BOX_CLIENT_SECRET environment variables',
        },
        { status: 503 }
      )
    }
    ...
    const items = folder.item_collection.entries.map((item: BoxItem) => {
    ...
    items.sort(
      (a: { lastModified?: string }, b: { lastModified?: string }) =>
